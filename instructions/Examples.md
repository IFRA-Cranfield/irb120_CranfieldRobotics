...
This environment does not have any particular use/application, but simply visualizing the IRB120 robot and its end-effector and enclosure in the Simulation Environment. Execute the following command to launch a ROS 2-Gazebo Simulation Environment of the IRB120-Cranfield Robot:
...
For more detailed instructions on how to properly set up any ABB robot for ROS 2 and connect to the ABB IRB-120 robot, please see this [link-TBD]. Once that is ready, you can execute the following command to launch the ABB's ROS 2 driver along with MoveIt!2, and our custom ROS 2 tools for robot operation:
...
# 3. Execute the Cube Pick&Place Robot Program:
ros2 run ros2srrc_execution ExecuteProgram.py package:=irb120cranfield program:=CubePP_irb120
```
